...
const device_bus_1 = require("./device-bus");
...
const parseDevices = () => {
    ...
    if (!raw || raw.trim().toLowerCase() === "all") {
        return [...protocol_1.DEVICE_IDS];
    }
    const requested = raw
        .split(",")
        .map((entry) => entry.trim().toLowerCase())
        .filter((entry) => protocol_1.DEVICE_IDS.includes(entry));
...
const bus = (0, device_bus_1.createDeviceBus)({
    brokerUrl,
...
if (simulatesActuator) {
    bus.onCommand("actuator", (command) => {
        if (command.command === "setDriveMode") {
            if (!sim_state_1.DRIVE_MODES.includes(command.value)) {
                console.warn(`[Device:actuator] modalita' rifiutata: ${command.value}`);
                return;
            }
            simulation.setDriveMode(command.value);
...
const timer = setInterval(() => {
    ...
    if (seq % 15 === 0 && bus.isConnected()) {
        console.log(`[Device] SoC ${simulation.state.batterySoC.toFixed(1)}% | ` +
